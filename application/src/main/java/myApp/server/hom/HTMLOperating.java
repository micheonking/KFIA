...
public class HTMLOperating implements javax.servlet.Servlet {
	...
	private String getHtml() {
		...
		for(String companyName : companyList) {
			...
			List<Hom03_OperatingModel> list = sqlSession.selectList("hom03_operating.selectByCodeName", companyName);	
			...
			for(int i = 0 ; i<list.size(); i++) {
				...
				if(i == 0) {
					rowString += this.tdRowSpanGrey(list.size(), operatingModel.getOrgCodeId()+"", 50, "center"); 	
				}
